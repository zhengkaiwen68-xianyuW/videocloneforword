...
从 rewrite/minimax_adapter.py 迁移，实现 LLMProvider Protocol。
"""
...
class MiniMaxAdapter:
    ...
    实现 LLMProvider Protocol，同时提供域特定方法（rewrite, generate_hook 等）。
    """
    ...
    async def generate(
        ...
        """调用 MiniMax API 生成文本（带指数退避重试和并发控制）"""
        from ..core.concurrency import concurrency_limiter

        # 获取 LLM 并发槽位（阻塞等待直到有空位）
        await concurrency_limiter.acquire_llm(caller=f"minimax:{self.model}")
        ...
        try:
            ...
            # 释放 LLM 并发槽位
            concurrency_limiter.release_llm(caller=f"minimax:{self.model}")
