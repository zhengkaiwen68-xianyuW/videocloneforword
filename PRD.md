...
| **选题技法分析** | 从多篇视频提炼选题角度、痛点图谱、选题公式 | P0 |
| **黄金3秒钩子拆解** | 拆解视频开头 Hook，提取7种钩子类型和心理机制 | P0 |
| **内容结构映射** | 分析信息密度曲线、情绪操控节点、CTA模式 | P1 |
| **技法驱动重写** | 使用「技法驱动+风格驱动」双轨 Prompt 重写 | P0 |
| **并发控制** | Semaphore 限制任务/LLM/下载并发数 | P1 |
| **API 限流** | 基于 IP 的滑动窗口限流，防止高频轮询 | P1 |
...
+---------------------------------------------------------------------+
|                         Web UI                                      |
|  (人格管理 / 文本重写 / 技法库浏览 / 配置面板 / 历史记录)                |
+-----------------------------+---------------------------------------+
                              | HTTP API (+ Rate Limit Middleware)
+-----------------------------v---------------------------------------+
|                      FastAPI Server                                 |
|  +-------------+  +-------------+  +-------------+  +------------+ |
|  |  人格管理   |  |   重写服务   |  |   配置管理   |  |  技法 API  | |
|  +------+------+  +------+------+  +-------------+  +------+-----+ |
|         |                |                                  |       |
|  +------v----------------v----------------------------------v-----+ |
|  |                    ConcurrencyLimiter                          | |
|  |          (任务/LLM/下载 三级 Semaphore + API 限流)               | |
|  +-------------------------------+-------------------------------+ |
|                                  |                                  |
|  +-------------------------------v-------------------------------+ |
|  |                     Persona Engine                            | |
...
+-----------------------------+---------------------------------------+
                              |
+-----------------------------v---------------------------------------+
|              SQLite Database (StaticPool + WAL)                    |
|  +----------+ +---------------+ +----------+ +------------------+  |
|  | personas | | rewrite_tasks | |  hooks   | | topic_techniques |  |
|  +----------+ +---------------+ +----------+ +------------------+  |
+---------------------------------------------------------------------+
```
...
用户不仅需要"模仿他怎么说话"，更需要"学会他怎么创作"。技法提炼引擎从表层风格克隆升级为深层内容技法提炼，实现：
- 选题技法分析（角度偏好、痛点图谱、选题公式）
- 黄金3秒钩子拆解（7种钩子类型、心理机制、结构公式）
- 内容结构映射（信息密度曲线、情绪操控节点、CTA模式）

### 4.2 钩子类型分类

| 类型 | 枚举值 | 公式 | 心理机制 |
|------|--------|------|---------|
| 反逻辑 | `reverse_logic` | {常识} + 根本不是{常识} | 认知失调 |
| 痛点刺痛 | `pain_point` | 直接戳焦虑 | 损失厌恶 |
| 利益炸弹 | `benefit_bomb` | 极低成本 + 极高收益 | 即时满足 |
| 悬念断句 | `suspense_cutoff` | 话说一半 | 好奇心缺口 |
| 权威颠覆 | `authority_subvert` | 借权威反权威 | 权威悖论 |
| 数据冲击 | `data_impact` | 用数字制造震撼 | 社会比较 |
| 身份标签 | `identity_label` | 给观众贴标签 | 身份认同 |

### 4.3 技法驱动重写流程

```
原始素材 → 选题技法匹配 → 钩子类型选择 → 结构映射注入 → 风格约束叠加 → LLM 重写 → 评分验证
                ↑               ↑              ↑              ↑
          TopicTechnique   HookAnalysis   ContentStructure  PersonalityProfile
```
...
| 方法 | 路径 | 说明 |
|------|------|------|
| `POST` | `/v1/personas/{id}/analyze-techniques` | 触发技法提炼 |
| `GET` | `/v1/personas/{id}/techniques` | 获取技法画像 |
| `GET` | `/v1/hooks` | 查询技法库 |
| `GET` | `/v1/personas/{id}/hook-stats` | 钩子统计 |
...
| 资源 | 默认限制 | 说明 |
|------|---------|------|
| 视频处理任务 | 3 | 超出返回 HTTP 429 |
| LLM API 调用 | 5 | 排队等待 |
| B站下载 | 2 | 避免触发反爬 |
| API 请求 | 60/min/IP | 滑动窗口限流 |

### 5.2 实现方案

- 基于 `asyncio.Semaphore` 的轻量并发控制
- 任务创建时检查槽位（ASR 任务立即拒绝，人格创建排队等待）
- LLM 调用使用 `acquire_llm/release_llm` 控制
- API 限流中间件基于 IP 滑动窗口
